...
CChartsDrawer.prototype =
{
	...
	_calculateMarginsChartOrdinaryDiagrams: function (chartTitle, axes, axesTitles, legendTexts) {
		...
		const labelBasedMarginTop = legendTexts?.top ? legendTexts.top + marginWithOffsetApplied2 : 0;
		const labelBasedMarginLeft = legendTexts?.left ? legendTexts.left + marginWithOffsetApplied2 : 0;
		const labelBasedMarginRight = legendTexts?.right ? legendTexts.right + marginWithOffsetApplied2 : 0;
		const labelBasedMarginBottom = legendTexts?.bottom ? legendTexts.bottom + marginWithOffsetApplied2 : 0;
