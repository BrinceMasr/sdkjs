...
$(function ()
{
	QUnit.module("ApiName");

	QUnit.test("SetRefersTo", function (assert) {
		AscTest.JsApi.AddDefName("TestSetRefersTo", "Sheet1!$A$1:$B$2");
		const name = AscTest.JsApi.GetDefName("TestSetRefersTo");

		assert.strictEqual(name.SetRefersTo("Sheet1!$C$1:$D$2"), true, "SetRefersTo returns true");
		assert.strictEqual(name.GetRefersTo(), "Sheet1!$C$1:$D$2", "GetRefersTo returns the new reference");
	});

	QUnit.test("Delete", function (assert) {
		AscTest.JsApi.AddDefName("TestDeleteName", "Sheet1!$A$1");
		const name = AscTest.JsApi.GetDefName("TestDeleteName");

		assert.strictEqual(name.Delete(), true, "Delete returns true");
		assert.throws(
			function () { AscTest.JsApi.GetDefName("TestDeleteName"); },
