...
CPresentation.prototype.InsertSlidesFromBinary = function (binary, imageMap, index, slideStart, slideEnd, keepSourceFormatting) {
	if (!binary) {
		return 0;
	}

	const drawingDocument = this.DrawingDocument;
	const urlsData = {imageMap: imageMap || {}, documents: [], convertCallback: function () {}, endCallback: function () {}};
	const oldInsertUrls = Asc.editor.insertDocumentUrlsData;
	Asc.editor.insertDocumentUrlsData = urlsData;

	const rebuildImages = [];
	const tempPres = AscFormat.ExecuteNoHistory(function () {
		const pres = new CPresentation(drawingDocument, false);
		const loader = new AscCommon.BinaryPPTYLoader();
		loader.Api = Asc.editor;
		loader.Start_UseFullUrl(urlsData);
		AscCommon.pptx_content_loader.Start_UseFullUrl(urlsData);
		const ok = false !== loader.Load(binary, pres);
		const embeddedImages = AscCommon.pptx_content_loader.End_UseFullUrl();
		const topLevelImages = loader.End_UseFullUrl();
		if (topLevelImages && topLevelImages.length) rebuildImages.push.apply(rebuildImages, topLevelImages);
		if (embeddedImages && embeddedImages.length) rebuildImages.push.apply(rebuildImages, embeddedImages);
		return ok && pres.Slides.length > 0 ? pres : null;
	}, this, []);

	if (tempPres && rebuildImages.length) {
		const native = typeof window !== "undefined" ? window["native"] : null;
		if (native && typeof native["GetImageUrl"] === "function") {
			AscFormat.ExecuteNoHistory(function () {
				for (let i = 0; i < rebuildImages.length; ++i) {
					const builderImg = rebuildImages[i];
					if (!builderImg || !builderImg.Url) continue;
					const localName = native["GetImageUrl"](builderImg.Url);
					if (typeof localName === "string" && localName.length > 0) {
						builderImg.SetUrl(localName);
					}
				}
			}, null, []);
		}
	...
	if (!tempPres) {
		Asc.editor.insertDocumentUrlsData = oldInsertUrls;
		return 0;
	}

	const srcCount = tempPres.Slides.length;
	let start = AscFormat.isRealNumber(slideStart) && slideStart >= 0 ? slideStart : 0;
	let end = AscFormat.isRealNumber(slideEnd) && slideEnd >= 0 ? slideEnd : srcCount - 1;
	if (end >= srcCount) end = srcCount - 1;
	if (start > end) {
		Asc.editor.insertDocumentUrlsData = oldInsertUrls;
		return 0;
	}

	let pos = AscFormat.isRealNumber(index) ? index : this.Slides.length;
	if (pos < 0) pos = 0;
	if (pos > this.Slides.length) pos = this.Slides.length;

	const srcW = tempPres.GetWidthMM();
	const srcH = tempPres.GetHeightMM();
	const kw = AscFormat.fApproxEqual(this.GetWidthMM(), srcW) ? 1.0 : this.GetWidthMM() / srcW;
	const kh = AscFormat.fApproxEqual(this.GetHeightMM(), srcH) ? 1.0 : this.GetHeightMM() / srcH;
	const needsResize = kw !== 1.0 || kh !== 1.0;
	const keepSource = true === keepSourceFormatting;

	const destMasterAnchor = this.Slides[pos > 0 ? pos - 1 : 0];
	const destMaster = destMasterAnchor && destMasterAnchor.Layout ? destMasterAnchor.Layout.Master : this.slideMasters[0];

	const masterMap = {};
	const layoutMap = {};
	const insertedCount = end - start + 1;

	for (let i = 0; i < insertedCount; ++i) {
		const srcSlide = tempPres.Slides[start + i];
		const srcLayout = srcSlide.Layout;
		const srcMaster = srcLayout ? srcLayout.Master : null;

		let targetLayout = null;
		if (keepSource) {
			if (srcMaster) {
				const srcMasterId = srcMaster.Get_Id();
				let targetMaster = masterMap[srcMasterId];
				if (!targetMaster) {
					targetMaster = srcMaster.createDuplicate();
					if (needsResize) {
						targetMaster.scale(kw, kh);
					}
					targetMaster.setSlideSize(this.GetWidthMM(), this.GetHeightMM());
					for (let lt = 0; lt < srcMaster.sldLayoutLst.length; ++lt) {
						const srcLtId = srcMaster.sldLayoutLst[lt].Get_Id();
						layoutMap[srcLtId] = targetMaster.sldLayoutLst[lt];
					}
					this.pushSlideMaster(targetMaster);
					masterMap[srcMasterId] = targetMaster;
				}
			}
			targetLayout = srcLayout ? layoutMap[srcLayout.Get_Id()] : null;
		} else if (destMaster) {
			if (srcLayout) {
				targetLayout = destMaster.getMatchingLayout(srcLayout.type, srcLayout.matchingName, srcLayout.cSld.name, true);
			}
			if (!targetLayout) {
				targetLayout = destMaster.sldLayoutLst[0];
			}
		}

		const slide = srcSlide.createDuplicate();
		if (needsResize) {
			slide.Width = srcW;
			slide.Height = srcH;
			slide.changeSize(this.GetWidthMM(), this.GetHeightMM());
		}
		if (targetLayout) {
			slide.setLayout(targetLayout);
		}
		if (!keepSource) {
			slide.changeBackground(null);
		}

		if (!slide.notes) {
			slide.setNotes(AscCommonSlide.CreateNotes());
		}
		slide.notes.setNotesMaster(this.notesMasters[0]);
		slide.notes.setSlide(slide);

		this.insertSlide(pos + i, slide);
	}
